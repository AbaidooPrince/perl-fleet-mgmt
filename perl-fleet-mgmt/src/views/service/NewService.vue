    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" md="12" class="pb-0">
              ...
              <div>
              <label>Odometer<strong class="text--error"></strong></label>
              <v-text-field
              v-model="serviceForm.odometer"
              dense
              outlined
              persistent-hint
              >
              ...
              <label>Completion Date</label>
                ...
                <div>
              <label>Starting Date (if applicable)</label>
                  <v-row>
                    <v-col cols="12" md="4" lg="4" sm="12">
                    <v-menu
                      ref="menu3"
                    ...
                    >
                      ...
                      <v-date-picker v-model="startingDate" no-title scrollable>
                        ...
                        <v-btn
                          text
                          color="primary"
                          @click="$refs.menu3.save(startingDate)"
                        >
                          OK
                        </v-btn>
                      </v-date-picker>
                    ...
                    >
                      ...
                      <v-time-picker v-model="startingTime" no-title scrollable>
                        <v-spacer></v-spacer>
                        ...
                        <v-btn
                          text
                          color="primary"
                          @click="$refs.menu4.save(startingTime)"
                        >
                          OK
                        </v-btn>
                      </v-time-picker>
                ...
                <div>
              <label>Vendor<strong class="text--error">*</strong></label>
              <v-text-field
              v-model="serviceForm.vendor"
              dense
    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" md="12" class="pb-0">
                ...
                <v-divider></v-divider>
                <div v-if="serviceForm.issues.length === 0" class="d-flex align-items-center justify-content-center no-item-row">
                  <div class="">
                ...
                <template v-slot:[`item.labor`]="{item, index}">
                  ...
                  dense
                  outlined
                  v-model="serviceForm.issues[index].labor"
                  ></v-text-field>
                </template>
                <template v-slot:[`item.parts`]="{item, index}">
                  ...
                  dense
                  outlined
                  v-model="serviceForm.issues[index].parts"
                  ></v-text-field>
                </template>
...
export default {
  ...
  methods: {
    async uploadToServer (e) {
      ...
      if (response && this.isDoc) {
        this.serviceForm.documents.push({ url: response, name: fileList.name })
        this.uploading = false
      } else {
        this.serviceForm.photos.push({ url: response, name: fileList.name })
        this.uploading = false
      }
    },
    triggerUpload (type) {
    ...
    removeIssue (index) {
      this.serviceForm.issues.splice(index)
    },
    getSubtotal (item) {
    ...
    addService () {
      if (!this.serviceForm.vehicleId) return
      if (!this.$refs.servce_form.validate()) return
      this.serviceForm.startDate = `${this.serviceForm.startingDate}T${this.serviceForm.startTime}:00Z`
      this.serviceForm.endDate = `${this.serviceForm.endingDate}T${this.serviceForm.endTime}:00Z`
      // console.log('date', this.startDate)
      const response = this.$store.dispatch('services/addService', this.serviceForm)
      if (response) {
        // this.dialog = false
  ...
  computed: {
    servicePhotos: {
      get () {
        return this.serviceForm.photos
      }
    },
    serviceDocs: {
      get () {
        return this.serviceForm.documents
      }
    },
    allVehicles: {
      get () {
